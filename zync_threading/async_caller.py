...
class AsyncCaller(object):
  ...
  def __init__(self, thread_pool, thread_synchronization_factory):
    self._thread_pool = thread_pool
    self._async_tasks = set()
    self._async_tasks_lock = thread_synchronization_factory.create_lock()
  ...
  def start_async_call(self, func, on_success=None, on_error=None, name=None):
    ...
    raising exceptions. The return value is passed to this callback.
    :param Optional[(BaseException, str) -> None] on_error: (Optional) Called when func raises an
    exception. The exception is passed to this callback along with the traceback.
    :param Optional[str] name: (Optional) Sets the name of the task that will execute this call.
    Name is passed to thread pool error handlers to help identify error causes.
    ...
    class _AsyncTask(BaseInterruptibleTask):
      ...
      def run(self):
        ...
        try:
          ...
          if on_error:
            on_error(err, traceback.format_exc())
          else:
            raise
        finally:
  ...
  def async_call(on_success=None, on_error=None, name=None):
    ...
    Arguments of the decorator should be methods, they will be bound to `self` of the wrapped
    method.
    ...
    :param Optional[(object) -> None] on_success: (Optional) Called when func returns without
    raising exceptions. The return value is passed to this callback.
    :param Optional[(BaseException) -> None] on_error: (Optional) Called when func raises an
    exception. The exception is passed to this callback along with the traceback.
    :param Optional[str] name: (Optional) Sets the name of the task that will execute this call.
    Name is passed to thread pool error handlers to help identify error causes.
    :return function:
